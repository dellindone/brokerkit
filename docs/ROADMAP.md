...
> **▶ NEXT SESSION STARTS HERE:** 3.2 — `OrderRequest` + `Order` models in `brokerkit-core/brokerkit/models/order.py`. Opening design question for the user (they were told to think about it): why two models, not one — what a strategy *asks* (OrderRequest) vs what the broker *reports* (Order: ids, status, filled qty, avg price). 3.0+3.1 done & committed (b118901): 6 order enums verified against SDK/docs, F&O-aware Instrument, segment filter, Groww quirk normalization (futures strike=0→None, MCX fake ISINs→None).
